const project = {
    ...
    handleFormUpdate(){
        ...
        if(project.modalUpdate) project.modalUpdate.style.display = (project.modalUpdate.style.display === 'flex') ? 'none' : 'flex'
        })
    },
    handleFormDelete(){
        project.allBtn[2].addEventListener('click', (event) =>{
        if(project.modalDelete) project.modalDelete.style.display = (project.modalDelete.style.display === 'flex') ? 'none' : 'flex'
        })
    },
